...
import { Search, Loader2, ListFilter, Users, ArrowRight, Share2, Crown } from "lucide-react";
        ...
        <div className="animate-fade-up">
            <div className="flex flex-col gap-6 mb-8">
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="flex-1 relative group">
                            ...
                            <input
                                ...
                                className="w-full bg-zinc-900/50 border border-white/5 rounded-2xl py-4 pl-14 pr-6 text-white text-sm font-medium focus:outline-none focus:border-white/30 transition-all placeholder:text-zinc-600 shadow-2xl"
                            />
                        </div>
                        <button
                    ...
                    {/* Expanded Filters */}
                    {showFilters && (
                        <div className="p-8 bg-zinc-900/40 backdrop-blur-3xl border border-white/10 rounded-[32px] grid md:grid-cols-3 gap-10 animate-in slide-in-from-top-4 duration-300 shadow-2xl">
                            <div>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 block">Min. Reach</label>
                ...
                <div className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest ml-2">
                    {loading ? "Scanning for voices..." : `${creators.length} creator${creators.length === 1 ? '' : 's'} available`}
                </div>
            </div>
            ...
            {loading ? (
                <div className="py-20 flex flex-col items-center justify-center space-y-4">
                    ...
                    <span className="text-xs font-black text-gray-500 uppercase tracking-widest">Accessing Network...</span>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {!loading && creators.length === 0 && (
                        <div className="col-span-full py-40 text-center glass-panel border-dashed border-white/5 flex flex-col items-center">
                            ...
                            <p className="text-sm text-gray-700 font-bold uppercase tracking-wider">Try adjusting your tactical search parameters.</p>
                        </div>
                    )}
                </div>
            )}
...
function CreatorCard({ creator, index, onClick }: { creator: any, index: number, onClick: () => void }) {
    ...
    return (
        <div
            onClick={onClick}
            className="group glass-panel p-6 cursor-pointer hover:border-white/20 transition-all duration-300 animate-fade-up flex flex-col gap-4 shadow-2xl"
            style={{ animationDelay: `${index * 50}ms` }}
        >
            <div className="flex items-start gap-4">
                <div className="relative shrink-0">
                    <div className="w-12 h-12 rounded-full overflow-hidden border border-white/10 bg-[#0a0f1a] flex items-center justify-center text-lg font-bold text-white relative shadow-xl">
                        {creator.avatar_url ? (
                ...
                <div className="flex-1 min-w-0 py-0.5">
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="font-bold text-white text-base truncate pr-2">{creator.display_name}</h3>
                        <span className="text-[9px] px-2 py-0.5 rounded-lg bg-white/5 text-zinc-500 border border-white/10 font-black uppercase tracking-widest group-hover:bg-primary group-hover:text-white group-hover:border-primary/20 transition-all shrink-0">
                            {creator.niche}
                        </span>
                    </div>
                    <p className="text-xs text-zinc-500 font-medium line-clamp-2 leading-relaxed opacity-80">
            ...
            <div className="flex items-center justify-between pt-3 border-t border-white/5">
                <div className="flex items-center gap-2 text-zinc-500 font-bold">
                    ...
                    <span className="text-[10px] uppercase tracking-widest">
                        {((getTotalFollowers(creator.creator_platforms) || 0) / 1000).toFixed(1)}K reached
                    </span>
                </div>

                <div className="flex items-center gap-3">
                    <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                        from <span className="text-white ml-1">${creator.min_budget || 100}</span>
                    </div>
                    <div className="p-1.5 rounded-full bg-white/5 text-zinc-400 group-hover:bg-white group-hover:text-black transition-all">
